<template>
    <div class="py-1 w-100">
        <div class="rounded bg-slab-primary mr-4 h-44">
        ...
        <div v-if="expanded" class="fixed top-0 left-0 bottom-0 right-0 flex items-center justify-center z-30" style="background:rgba(0,0,0,0.5)">
            <div class="absolute top-0 left-0 right-0 bottom-0 z-0" @click="expanded = false"></div>
            <div class="bg-slab rounded p-4 relative z-10 w-1/2" style="max-width: 800px;">
                ...
                <div v-if="policy.id == 'E3' || policy.id == 'E4' || policy.id == 'H4' || policy.id == 'H5'" class="text-3xl text-white font-bold">US${{policy.value | numeralFormat}}</div>

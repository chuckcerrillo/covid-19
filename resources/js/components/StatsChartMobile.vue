<template>
    <div class="absolute top-0 left-0 right-0 bottom-4" v-on:click="resetDropdowns()">
        ...
        <div v-if="!ui.settings" class="absolute bottom-0 inset-x-0 text-center p-4 bg-hoverslab text-xs" @click.stop="ui.settings = !ui.settings">Chart Settings</div>
        <div v-else class="absolute bottom-0 inset-x-0 text-center p-4 bg-slab text-xs" @click.stop="ui.settings = !ui.settings">Close</div>
    </div>
</template>
